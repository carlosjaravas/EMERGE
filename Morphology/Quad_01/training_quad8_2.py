...
def init_position():
    ...
    if start_pos == 0:
        ...
        init_pos = [0, math.pi/2, 0, math.pi/2, 0, math.pi/2, 0, math.pi/2]
        for joint_index in range(sim_obj.num_joints):
            sim_obj.sim.setJointTargetPosition(sim_obj.joint_handler_ids[joint_index], init_pos[joint_index])
            sim_obj.sim.setJointPosition(sim_obj.joint_handler_ids[joint_index], init_pos[joint_index])
        sim_obj.sim.startSimulation()
        sim_obj.client.step()
