...
def export():    
    ...
    sim_obj.file = open('training_quad8_dataset_90_' + str(port_conexion) + timestr + '.pkl', 'wb')
    pickle.dump(sim_obj.training_data, sim_obj.file)
    sim_obj.file.close()
    if not cluster:
        df = pd.DataFrame(sim_obj.training_data)
        df.to_csv('training_quad8_dataset_90_' + str(port_conexion) + timestr + '.csv', index=False)
